...
function getDate(data) {
  ...
  const date = execSync(`git log -C100% --follow --format="%ad" -- "${filePath}" | tail -1`).toString().trim();
...
function getUpdated(data) {
  ...
  const updated = execSync(`git log -C100% --follow --format="%ad" -- "${filePath}"  | sort -r | head -1`).toString().trim();
  if (updated === '') {
    return moment();
  }
